...
public partial class leave_details : System.Web.UI.Page
{
    ...
    private string GetNormalLeavesBaseQuery()
    {
        ...
        query = @"select leave_id, Emp_ID, Emp_Name, Leave_id,Leave_Name, FromDate, ToDate,  Status, Approval, hl_status, LeaveDetails_id, remarks, ApprovedbyName, row from 
                (SELECT L.Leave_id,L.EmpID as Emp_ID , E.Emp_Name as Emp_Name, LM.LeaveName as Leave_Name, LDate as FromDate , 
                EndDate as ToDate ,ls.Leave_Status_text as Status,Flag as Approval,l.hl_status, LeaveDetails_id, L.Remarks,
                L.ApprovedbyName, ROW_NUMBER() OVER (ORDER BY l.leave_id desc) as row FROM Leave1 L JOIN LeaveMaster LM ON L.LeaveType = LM.LeaveCode 
                join EmployeeMaster e on e.Emp_Code=l.empid join leave_status Ls on Ls.Leave_Status_id = L.Flag where e.emp_status = 1  ";
    ...
    public static ReturnObject GetNormalLeavesData(int page_number, bool is_filter, string filters)
    {
        ...
        try
        {
            ...
            query = page_object.GetNormalLeavesBaseQuery(); //read main query
    ...
    {
        string query = string.Empty;
        query = @"select leave_id, Emp_ID, Emp_Name, Leave_id,Leave_Name, FromDate, ToDate,  Status, Approval, hl_status, LeaveDetails_id, NoOfDays, remarks, ApprovedbyName, row from 
                ...
                ls.Leave_Status_text as Status,Flag as Approval, l.hl_status, LeaveDetails_id, L.noofdays[NoOfDays], L.Remarks, L.ApprovedbyName,
