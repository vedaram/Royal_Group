$(function () {
	...
	var Company = (function ($, w, d) {
		...
		function getCompanyData() {
			return SAXHTTP.AJAX(
				"detailed_report.aspx/GetCompanyData", {}
			)
	...
	var OtherData = (function ($, w, d) {
		...
		function getOtherData(company_code) {
			...
			return SAXHTTP.AJAX(
					"detailed_report.aspx/GetOtherData",
	...
	var ReportMasterView = (function ($, w, d) {
		...
		function _initOther() {
			...
			$to_date_ref = $to_date.Zebra_DatePicker({
				format: 'd-M-Y',
			...
			$('#from_date').Zebra_DatePicker({
				...
				onSelect: function (date) {
					...
					// reset the value of to date
					$to_date.val("");
					
					var selected_date = moment(date, "DD-MMM-YYYY"),
						day = selected_date.date(),
						to_date = selected_date.add(29, 'd').date(),
						to_date_month = selected_date.format("DD-MMM-YYYY"),
						to_date_ref = '',
						enabled_dates = '';

					enabled_dates = [to_date.toString()+' * * *']; 

					if (to_date > 28) {
						enabled_dates.push('28-29 02 * *');
					}
					if (to_date >= 30) {
						enabled_dates.push('30 04,06,09,11 * *');	
					}

					to_date_ref != '' ? $to_date.data('Zebra_DatePicker').destroy() : '';
					to_date_ref = $to_date.Zebra_DatePicker({
						format: 'd-M-Y',
						disabled_dates: ['* * * *'],
						enabled_dates: enabled_dates,
						start_date: to_date_month
					});
