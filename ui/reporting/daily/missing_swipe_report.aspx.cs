...
public partial class daily_missing_swipe_report : System.Web.UI.Page
{
    const string page = "DAILY_MISSING_SWIPE_REPORT";
    ...
    private void PrepareDataForExport(string filters, string branches)
    {
        ...
        if (company_name != "select")
        {
            ...
            where_clause += " and status='MS' and Emp_ID in (select EmpID from [FetchEmployees] ('" + current_user_id + "','" + employee_status + "'))";


            filter_conditions.Add("where", where_clause);

            db_connection.ExecuteStoredProcedureWithHashtable_WithoutReturn("MissingSwipeFilter1", filter_conditions);
        }
    ...
    private DataTable GetFilteredData()
    {
        ...
        query = "select TOP " + export_limit + " PDate, Emp_ID, Emp_Name,cat_name, dept_name ,EmployeeCategory  , shift_code , shift_name , status from missingswipereport";
    ...
    private string CreateExport(DataTable filtered_data, string company_name)
    {
        ...
        string[] column_names =
            new string[] { "Punch Date", "Employee ID", "Employee Name", "Branch", "Department","Category", "Shift Code", "Shift Name", "Status" };

       

        string importFile,
          user_id = HttpContext.Current.Session["employee_id"].ToString(),
          exportPath = "DailyMissingSwipeReport-" + user_id + "-" + now.ToString("yyyyMMdd_HHmmss") + ".xlsx";
        ...
        ExcelExport.ExportDataToExcelWithLogo(importFilePath, "DAILY MISSING SWIPE REPORT", filtered_data, Context, column_names, company_name, exportPath);
    ...
    public static ReturnObject DoExport(string filters, string branches)
    {
        ...
        daily_missing_swipe_report page_object = new daily_missing_swipe_report(); // instance of the page to access non static methods.
        ReturnObject return_object = new ReturnObject();
        DataTable filtered_data = new DataTable();
        JObject filters_data = new JObject();
